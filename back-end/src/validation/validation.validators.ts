...
export class ValidationValidators {
    ...
    static async validateEntity<T> (entity: T, serviceConfig: ServiceConfig): Promise<Array<ValidationError>> {
        ...
        validValidators.forEach(validator => {
            promises.push(validator.validate(entity, serviceConfig));
        });
    ...
    static async validatePayload<T> (payload: T, serviceConfig: ServiceConfig): Promise<Array<ValidationError>> {
        const validValidators = ValidationValidators.PAYLOAD_VALIDATORS.filter(validator => validator.isValidEntity(payload));
        const promises: Array<Promise<Array<ValidationError>>> = [];
        validValidators.forEach(validator => {
            promises.push(validator.validate(payload, serviceConfig));
        });
